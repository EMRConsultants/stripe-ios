...
public class STPFakeAddPaymentPassViewController: UIViewController {
  ...
  public override func viewDidLoad() {
    ...
    contentLabel.rightAnchor.constraint(equalTo: view.rightAnchor, constant: -10.0).isActive = true
    ...
    pairsTable?.heightAnchor.constraint(equalToConstant: CGFloat((rows.count * 50))).isActive = true
    state = .initial
  }
        ...
        completionHandler: { request in
          if self.state == .loading {
            ...
            if request.stp_error != nil {
              var error =
                (request.stp_error as NSError?)?.userInfo[STPError.errorMessageKey] as? String
              if error == nil {
                error =
                  (request.stp_error as NSError?)?.userInfo[NSLocalizedDescriptionKey] as? String
              }
